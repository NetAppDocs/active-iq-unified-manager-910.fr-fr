...
NOTE: Le port par défaut pour MySQL, 3306, est limité à localhost uniquement lors de l'installation d'Unified Manager sur les systèmes VMware vSphere. Cela n'a aucun impact sur les scénarios de mise à niveau où la configuration précédente est conservée. Cette configuration peut être modifiée et la connexion peut être mise à disposition d'autres hôtes via la console de maintenance.
